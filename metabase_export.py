    ...
    if hospitals:
        for hospital in hospitals:
            ...
            if 'demographics' in hospital_configuration:
                demographics_ids = str(
                    hospital_configuration['demographics']).split(",")
            if 'hiv_status' in hospital_configuration:
                hiv_status_ids = str(
                    hospital_configuration['hiv_status']).split(",")
            if 'outcomes' in hospital_configuration:
                outcomes_ids = str(
                    hospital_configuration['outcomes']).split(",")
            if 'outcomes_268' in hospital_configuration:
                outcomes_268kg_ids = str(
                    hospital_configuration['outcomes_268']).split(",")
            if 'temperatures' in hospital_configuration:
                temperatures_ids = str(
                    hospital_configuration['temperatures']).split(",")
            if 'maternals' in hospital_configuration:
                maternal_ids = str(
                    hospital_configuration['maternals']).split(",")
            ...
            try:
                ...
                logging.info(
                    'Something Wicked Happened During Activating Embedding')
            ...
            try:
                ...
                dashboard_ids = tuple(
                    map(int, str(hospital_configuration['dashboard_ids']).split(",")))
            ...
            for i, id in enumerate(ids):
                ...
                try:
                    if export_dir is not None:
                        ...
                        if str(id) in demographics_ids:
                            ...
                            if id is not None and str(id) !='None':
                    ...
                    logging.info(
                        'Something Wicked Happened During Images Exportation.')
                    logging.exception(e)
                    sys.exit(1)
                ...
                try:
                    ...
                    if demographics_ids and demographics:
                        exports['demographics'] = sorted(demographics, key=lambda x: list(
                            map(int, demographics_ids)).index(int(''.join(d for d in x if d.isdigit()))))
                    if hiv_status_ids and hiv_status:
                        exports['hiv_status'] = sorted(hiv_status, key=lambda x: list(
                            map(int, hiv_status_ids)).index(int(''.join(d for d in x if d.isdigit()))))
                    if outcomes_ids and outcomes:
                        exports['outcomes'] = sorted(outcomes, key=lambda x: list(
                            map(int, outcomes_ids)).index(int(''.join(d for d in x if d.isdigit()))))
                    if outcomes_268kg_ids and outcomes_268kg:
                        exports['outcomes_268kg'] = sorted(outcomes_268kg, key=lambda x: list(
                            map(int, outcomes_268kg_ids)).index(int(''.join(d for d in x if d.isdigit()))))
                    if temperatures_ids and temperatures:
                        exports['temperatures'] = sorted(temperatures, key=lambda x: list(
                            map(int, temperatures_ids)).index(int(''.join(d for d in x if d.isdigit()))))
                    if maternal_ids and maternals:
                        exports['maternals'] = sorted(maternals, key=lambda x: list(
                            map(int, maternal_ids)).index(int(''.join(d for d in x if d.isdigit()))))
                    # Write The Json To The File
                    try:
                        json_file = json.dumps(exports)
                        file = open(
                            "{0}metabase_exports.json".format(export_dir), "w")
